...
Sorting keeps the **old** row numbers, which can be confusing.  
`.reset_index(drop=True)` gives you clean 0..N–1 row numbers after sorting.
...
```{python}
#| label: nicer-aggregate
#| echo: false

import pandas as pd
import numpy as np

# --- safety: rebuild minimal inputs if kernel was restarted ---
if "clean_shipments" not in globals():
    shipments_df = pd.read_csv(
        "https://raw.githubusercontent.com/flyaflya/persuasive/main/shipments.csv",
        parse_dates=["plannedShipDate","actualShipDate"]
...
# --- explicit, robust metrics (no named-agg) ---
total_shipments = len(clean_shipments)
late_count      = int(clean_shipments["is_late"].sum())
late_rate       = float(clean_shipments["is_late"].mean())              # 0..1
on_time_pct     = (1.0 - late_rate) * 100.0
avg_days_late   = float(clean_shipments["days_late"].mean())
max_days_late   = float(clean_shipments["days_late"].max())

display_tbl = pd.DataFrame([{
    "Shipments":          total_shipments,
    "Late shipments":     late_count,
    "Late rate":          late_rate,
    "On-time rate (%)":   on_time_pct,
    "Avg days late":      avg_days_late,
    "Max days late":      max_days_late,
}]).round({"Late rate": 3, "On-time rate (%)": 1, "Avg days late": 2})

display(display_tbl)
```


*Note:* Negative **“Avg days late”** means the shipment was **early** (went out before the planned date).
...
# --- build service_by_category if it's not already present ---
def _build_service_by_category():
    global service_by_category, shipments_with_category
    if "shipments_with_category" not in globals():
        # minimal rebuild
        shipments_df = pd.read_csv(
            "https://raw.githubusercontent.com/flyaflya/persuasive/main/shipments.csv",
        ...
        shipments_with_category = swl.merge(product_line_df, on="partID", how="left")
...
if "service_by_category" not in globals():
    _build_service_by_category()
...
### Conclusion
Across ~4k shipments, about **11%** were late. Categories differ: top groups run at ~95% on-time, while others are closer to **82%**. About **23%** of orders span multiple categories, which increases coordination needs.  
**Next steps:** prioritize SLAs/process fixes for the lowest on-time categories (especially **Machines** and **SpareParts**), and pilot a streamlined pick/pack flow for **multi-category** orders to reduce delays.
